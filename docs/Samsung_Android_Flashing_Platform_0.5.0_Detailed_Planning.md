...
5. **Cross-Platform Elevation Helper:** If we are to automate the driver injection (WinUSB/udev), the application must contain an OS-agnostic helper to request admin/root elevation silently or via explicit OS prompts (e.g., UAC for Windows, `pkexec`/`sudo` for Linux).
6. **MANIFEST.in / Wheel Construction:** We must update the `MANIFEST.in` and `pyproject.toml` data rules to ensure that the `libusb-1.0.dll` binary and any driver injector binaries are successfully packaged into the final PyPI wheel payload.