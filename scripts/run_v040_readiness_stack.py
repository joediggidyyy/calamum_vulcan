"""Run the Sprint 0.4.0 multi-strategy readiness stack for Calamum Vulcan."""
...
ARCHIVE_ROOT = REPO_ROOT / 'temp' / 'fs4_readiness'


@dataclass(frozen=True)
class ValidationLane:
  """One runnable validation lane in the Sprint 4 readiness stack."""
...
def build_default_lanes(include_testpypi: bool = False) -> Tuple[ValidationLane, ...]:
  """Return the default ordered Sprint 4 readiness lanes."""
...
def _render_markdown_summary(summary: Mapping[str, object]) -> str:
  lines = [
    '# Sprint 0.4.0 readiness summary',
    '',
  ...
  lines.extend(
    [
      ...
      '',
      '- `passed` means the selected Sprint 4 readiness lanes all completed successfully.',
      '- `failed` means at least one selected lane failed and Sprint 4 readiness should be treated as blocked until the failing lane is resolved or intentionally skipped with rationale.',
      '',
      '## Strategy coverage',
      '',
