"""Audit current implementation against the planned sprint timeline through 0.4.0."""
...
DEFAULT_OUTPUT_ROOT = REPO_ROOT / 'temp' / 'v040_timeline_audit'
RAW_DIR_NAME = 'raw'
...
SPRINT4_EVIDENCE_PATH = DOCS_ROOT / 'Samsung_Android_Flashing_Platform_0.4.0_Execution_Evidence.md'
SPRINT4_READINESS_PLAN_PATH = DOCS_ROOT / 'Samsung_Android_Flashing_Platform_0.4.0_Testing_and_Readiness_Plan.md'
SPRINT4_CHECKLIST_PATH = DOCS_ROOT / 'Samsung_Android_Flashing_Platform_0.4.0_Closeout_and_Prepackage_Checklist.md'
READINESS_SUMMARY_PATH = REPO_ROOT / 'temp' / 'fs4_readiness' / 'readiness_summary.json'
READINESS_SUMMARY_MARKDOWN_PATH = REPO_ROOT / 'temp' / 'fs4_readiness' / 'readiness_summary.md'
...
def _collect_readiness(
  ...
  if refresh:
    _, capture = _run_command(
      ...
      (sys.executable, 'scripts/run_v040_readiness_stack.py'),
      raw_root,
      check=False,
    )
    captures.append(capture)
...
def _build_criteria(context: Mapping[str, Any]) -> Tuple[CriterionResult, ...]:
  ...
  package_boundary_aligned = (
    str(pyproject['project']['version']) == '0.4.0'
    and 'local package-only Sprint 4 boundary' in readme_text
    and '## 0.4.0 - 2026-04-22' in changelog_text
  )
  publication_deferral_explicit = (
    'Do **not** publish `0.4.0` to PyPI' in checklist_text
  ...
  criteria = [
    ...
    CriterionResult(
      ...
      status='implemented',
      priority='high',
      impact='The core Sprint 4 extraction seam is live rather than prose-only.',
      evidence=(
    ...
    CriterionResult(
      ...
      status='implemented',
      priority='high',
      impact='Sprint 4 now owns one real, narrow execution lane instead of keeping safe-path strictly aspirational.',
      evidence=(
    ...
    CriterionResult(
      ...
      notes=(
        'Real-hardware transcript growth can continue as carry-forward debt, but Sprint 4 no longer depends on a single canned detect transcript.',
      ),
    ),
    CriterionResult(
      sprint='0.4.0',
      criterion_id='S4-05',
      title='The GUI workflow now reflects the settled Sprint 4 control flow',
      status='implemented',
      priority='high',
      impact='The operator shell now owns the truthful Sprint 4 deck progression instead of teaching one workflow while leaving the real path hidden in CLI-only surfaces.',
      evidence=(
    ...
    CriterionResult(
      sprint='0.4.0',
      criterion_id='S4-06',
      title='Safe-path closeout evidence and the Sprint 4 readiness orchestrator are implemented',
      status='implemented',
      priority='high',
      impact='Sprint 4 now has real integrated closeout proof and a repeatable multi-lane readiness sweep.',
      evidence=(
      ...
      notes=(
        'If the readiness summary is green, Sprint 4 broad validation is no longer hypothetical.',
      ),
    ),
    CriterionResult(
      sprint='0.4.0',
      ...
      priority='high',
      impact='Sprint 4 now closes as a local package boundary while preserving public publication as a later promotion gate instead of pretending unfinished release work is part of the sprint definition.',
      evidence=(
        'The closeout checklist explicitly says `Do **not** publish `0.4.0` to PyPI`.',
        'README now distinguishes the local `0.4.0` package-only boundary from the public `0.3.0` release.',
        'The readiness plan now treats TestPyPI/PyPI rehearsal as deferred carry-forward work rather than Sprint 4 acceptance criteria.',
      ),
      next_actions=(
        'Keep the publication workflow dormant until the immediate post-`0.6.0` `1.0.0` promotion gate if any doc surface still implies Sprint 4 depends on public upload.',
      ) if not publication_deferral_explicit else (),
    ),
    CriterionResult(
      sprint='0.4.0',
      criterion_id='S4-08',
      title='The local 0.4.0 package boundary metadata and readiness proof are aligned',
      status=s4_08_status,
      priority='critical',
      impact='Sprint 4 closeout is only honest when the repository version, release surfaces, and broad validation evidence all agree on the local package boundary being sealed.',
      evidence=(
        'pyproject now reports version `{version}`.'.format(
          version=pyproject['project']['version'],
        ),
        'README now frames `0.4.0` as the local package-only boundary while keeping `0.3.0` as the public release.',
        'CHANGELOG now includes the local `0.4.0` boundary section.',
      ...
      next_actions=(
        'Re-run the full Sprint 4 readiness stack until all selected lanes are green.',
      ) if not readiness_green else (
        'Finish aligning any remaining tracked release surface to the local `0.4.0` boundary if metadata still diverges.',
...
def _build_opportunistic_finds(context: Mapping[str, Any]) -> Tuple[Finding, ...]:
  return (
    Finding(
      ...
      severity='medium',
      category='extra',
      summary='Sprint 4 closeout proof is broader than the stale planning shells imply.',
      evidence=(
    ...
    Finding(
      ...
      severity='medium',
      category='extra',
      summary='The control-deck narrowing work is ahead of the old “planned only” narrative for the Sprint 4 operator lane.',
      evidence=(
    ...
    Finding(
      finding_id='X-04',
      title='Windows runtime ergonomics were hardened as opportunistic Sprint 4 gains',
      severity='low',
      category='extra',
...
def _build_deviations(context: Mapping[str, Any]) -> Tuple[Finding, ...]:
  ...
  if any(
    ...
    deviations.append(
      Finding(
        finding_id='D-01',
        title='Sprint 4 evidence ledger still understates implemented work',
        severity='high',
        category='doc-drift',
        summary='The local execution-evidence register still understates the current Sprint 4 closeout state.',
        evidence=(
          'The evidence ledger still contains one or more planned rows for `FS4-06`, `FS4-07`, or `FS4-08`.',
          'The workspace now generates `safe-path-close`, runs `run_v040_readiness_stack.py`, and exposes the bounded execute lane.',
        ),
      )
    )
  if 'safe-path-close` bundle does not exist yet' in readiness_plan_text:
    deviations.append(
      Finding(
        finding_id='D-02',
        title='The Sprint 4 readiness plan still says safe-path-close does not exist yet',
        severity='high',
        category='doc-drift',
        ...
        evidence=(
          '`Samsung_Android_Flashing_Platform_0.4.0_Testing_and_Readiness_Plan.md` still says `the deterministic `safe-path-close` bundle does not exist yet`.',
...
def _render_markdown(summary: AuditSummary) -> str:
  lines = [
    '# Calamum Vulcan `0.4.0` execution-timeline audit',
    '',
  ...
  lines.extend(
    [
      ...
      '- `open` means the planned end-state remains unclosed for the current `0.4.0` target.',
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
  parser = argparse.ArgumentParser(
    description='Audit the current Calamum Vulcan implementation against the 0.1.0-0.4.0 sprint timeline.',
  )
  parser.add_argument(
    '--output-root',
  ...
  parser.add_argument(
    '--refresh-readiness',
    action='store_true',
    help='Re-run the Sprint 4 readiness stack before scoring the audit.',
  )
  parser.add_argument(
    '--captured-at-utc',
...
def main(argv: Optional[Sequence[str]] = None) -> int:
  ...
  json_path = output_root / 'v040_timeline_audit.json'
  markdown_path = output_root / 'v040_timeline_audit.md'
